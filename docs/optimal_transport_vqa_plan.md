...
| Training | `train.py` optimizes teacher-forced answer cross-entropy and selects `best.pt` by generated validation F1. `test.py` generates answers but recomputes image/question encodings for teacher-forced loss. | Begin with the same answer loss, then test answer-aware auxiliary supervision. Cache/reuse encodings during evaluation if useful. Keep generated metrics as the checkpoint criterion. |
...
| Metrics and diagnostics | `utils/metrics.py` reports normalized exact match and token F1. `diagnose_training.py` tests image/question reliance, prediction collapse, and answer distribution. | Keep these, add question-type slices, transport diagnostics, and speed/memory measurements. Re-run shuffled-image tests for OT. |
...
| --- | --- | --- |
| 0. Establish baseline | Prepare larger, image-disjoint GQA train/dev/test CSVs; record data counts, question/answer lengths and type mix. Run current SAN model with fixed seed and configuration. Save generated EM/F1, per-type scores, latency, throughput, peak memory, and shuffled-image sensitivity. | Reproducible dataset manifest and at least one complete baseline run; three seeds for final comparison. |
...
| 5. Final report | Evaluate the frozen selection once on the held-out test split. Report mean and standard deviation across seeds, paired bootstrap confidence interval for EM/F1 difference, parameter count, latency, throughput, and peak VRAM. | Claim improvement only if the accuracy interval excludes zero and the compute cost is acceptable for the intended deployment. |

Suggested primary metric: generated exact match on GQA, with generated token F1 secondary. Report answer-type breakdown to expose gains hidden by the average. The existing metrics are simple string metrics rather than an official multi-reference GQA scorer; record that distinction. A practical first target is a repeatable 2 percentage-point absolute EM gain over SAN with no collapse in image reliance. This is a decision threshold, **not** a prediction from the retrieval paper.
