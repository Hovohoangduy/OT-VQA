...
| Images | ViT's `[B,N,D]` hidden states must not be treated as channel-first feature maps. | Preserve token/feature order. |
...
| Frozen vision encoder | Calling `model.train()` can re-enable its dropout although its parameters are frozen. | Keep the frozen ViT encoder in evaluation mode. |
...
The main model uses ViT/SAN and has no OCR extraction or copy mechanism. Its ability to answer questions about scene text remains limited. The model is an architecture prototype, not a complete reproduction of a published training pipeline, and synthetic metrics are not accuracy measurements.
